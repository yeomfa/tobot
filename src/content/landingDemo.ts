...
export function showcaseDemo(language: Language): Demo {
  ...
  return {
    ...
    */
    code: [
      { blockId: 's1', text: `let ${t.pointsVar} = 0;` },
      { blockId: 's2', text: `for (let ${t.counter} = 1; ${t.counter} <= 3; ${t.counter}++) {` },
      {
        blockId: 's3',
        text: `${t.pointsVar} = ${t.pointsVar} + ${t.counter};`,
        indent: 1,
      },
      { blockId: 's2', text: '}' },
      { blockId: 's4', text: `if (${t.pointsVar} >= 5) {` },
      { blockId: 's5', text: `console.log("${t.win}");`, indent: 1 },
      { blockId: 's4', text: '}' },
    ],
  };
}
