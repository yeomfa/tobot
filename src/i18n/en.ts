import type { Dictionary } from './es';

/**
 * Typed against the Spanish dictionary: adding a key there without adding it
 * here is a compile error, which keeps the two locales from drifting.
 */
export const en: Dictionary = {
  app: {
    name: 'Tobot',
    tagline: 'Learn to program by talking to a robot',
    untitled: 'Untitled algorithm',
  },
  actions: {
    selectedOne: '1 instruction',
    selectedCount: '{count} instructions',
    copyBlock: 'Copy',
    cut: 'Cut',
    paste: 'Paste',
    insertBefore: 'Insert before',
    insertAfter: 'Insert after',
    run: 'Run',
    stop: 'Stop',
    step: 'Step through',
    next: 'Step through',
    stepBack: 'Step back',
    exitStepping: 'Leave step-by-step',
    stepOne: 'Run one statement and wait',
    pause: 'Pause',
    resume: 'Resume',
    reset: 'Reset',
    clearConsole: 'Clear the console',
    clear: 'Clear',
    copy: 'Copy',
    copied: 'Copied',
    export: 'Export',
    delete: 'Delete',
    partActions: 'With this part',
    group: 'Group with the next',
    tools: 'Tools',
    groupTool: 'Group parts',
    groupHint: 'Drag across the parts that belong together',
    groupUnavailable: 'You need an expression of three parts or more',
    groupCount: 'Group {count} parts',
    ungroup: 'Remove the group',
    undo: 'Undo',
    redo: 'Redo',
    duplicate: 'Duplicate',
    moveUp: 'Move up',
    moveDown: 'Move down',
    addValue: 'Add value',
    addOperand: 'another part',
    removeOperand: 'Remove this part',
    addStatement: 'Add statement',
    addElseIf: 'add "else if"',
    addElse: 'Add "otherwise"',
    removeElse: 'Remove "otherwise"',
    rename: 'Rename',
    newAlgorithm: 'New algorithm',
    send: 'Send',
    close: 'Close',
    cancel: 'Cancel',
    confirm: 'Confirm',
    learnMore: 'Learn more',
    save: 'Save',
  },
  landing: {
    eyebrow: 'Programming from scratch',
    title: 'Program, without fighting the syntax',
    lead: 'Build algorithms out of blocks and watch them come alive.',
    tryIt: 'Start now',
    tryItLong: 'Open Tobot',
    signIn: 'Sign in',
    noAccount: 'Nothing to install. Start in the browser.',
    demoCaption: 'Tobot builds this algorithm and runs it, exactly as you will.',

    stepsTitle: 'How it works',
    stepsLead: 'Three steps, and the first one takes under a minute.',
    steps: {
      build: {
        title: 'Build your algorithm',
        body: 'Drag instructions in from the panel. Syntax errors do not exist here: blocks only fit together in ways that mean something.',
      },
      compare: {
        title: 'Understand what it does',
        body: 'You read it in plain English while it is written as real code. You learn to read it before having to write it.',
      },
      run: {
        title: 'Run it step by step',
        body: 'The robot goes one instruction at a time. You can step back, watch the variables and see where it went wrong.',
      },
    },

    featuresTitle: 'Everything to understand, nothing to memorise',
    features: {
        views: {
        title: 'From the idea to the code',
        body: 'You write in plain English and watch it become real code, step by step. The relationship between the two teaches itself.',
      },
      diagram: {
        title: 'Flowcharts',
        body: 'Drawn for you in the notation used in class. Export them for your report or your slides.',
      },
      run: {
        title: 'Forwards and back',
        body: 'Run step by step, go back whenever you like, and watch every variable change.',
      },
      robot: {
        title: 'A robot that answers',
        body: 'It speaks, asks you things and reacts to what you wrote. Programming stops being silent text.',
      },
      concepts: {
        title: 'Concepts explained',
        body: 'Every topic comes with a short explanation and links to trustworthy sources to go deeper.',
      },
      validation: {
        title: 'Errors in plain words',
        body: 'Tobot checks as you build and tells you what is missing in clear language, not compiler jargon.',
      },
      write: {
        title: 'And when you are ready, write it',
        body: 'Open your algorithm as real JavaScript and carry on from there. The robot comes too: it runs, it asks, and it tells you which line went wrong.',
      },
    },

    forTitle: 'Who is it for?',
    forStudents: {
      title: 'If you are learning',
      body: 'You understand what your code does before memorising how it is written. When you move to a real language, the logic is already yours.',
    },
    forTeachers: {
      title: 'If you teach',
      body: 'Hand out exercises as a link, with nothing to install in the lab. Your students see the diagram and the code of the same thing, side by side.',
    },


    robotHello: "Hi! I'm Tobot. Watch what I can do.",
    robotDone: "Done! That's an algorithm.",
    robotGuide: "I'll walk you through every step.",
    robotBye: 'Shall we?',

    sameTitle: 'Learn programming for real',
    journey: {
      logic: 'Logic',
      code: 'Code',
      concepts: 'Concepts',
      beyond: 'And whatever comes next',
    },
    views: {
      natural: 'English',
      pseudocode: 'Pseudocode',
      code: 'Real code',
      chart: 'Flowchart',
    },

    showcaseTitle: 'You write logic, not syntax',
    showcaseBody: 'You think through the solution and Tobot writes it as real code, while you learn to read it.',
    showcaseLabels: {
      blocks: 'Your blocks',
      natural: 'In English',
      code: 'In code',
    },

    readingTime: '{minutes} min',
    boxLabel: 'Inside Tobot',
    andMore: 'And more',
    sourcedFrom: 'Sourced from {publishers}.',
    exploreConcepts: 'Explore the concepts',
    learnTitle: 'Every topic, explained',
    learnLead: 'As you build, each instruction carries its explanation alongside it: the idea behind it, the common mistakes, and where to go deeper.',
    learn: {
      explained: 'Every topic explained in plain words',
      sources: 'Links to trustworthy sources to go deeper',
      pace: 'At your own pace, with nobody looking over your shoulder',
      growing: 'New topics as you go',
    },
    closingTitle: 'Shall we build your first algorithm?',
    closingBody: 'Open the editor and start playing. If you later want your work saved and available from any computer, you make an account.',
  },
  save: {
    /* At rest the work *is* saved — the last write finished. Same word
       as `saved` on purpose: the difference between them is the tick
       settling in, not the wording changing under the reader. */
    idle: 'Saved',
    saving: 'Saving…',
    saved: 'Saved',
    error: 'Not saved',
  },
  /** The panel above the robot, describing whatever is selected. */
  inspector: {
    title: 'Selection',
    problemCount: '{count} things to review',
  },
  palette: {
    variables: 'Variables in play',
    elementOf: 'item of {list}',
    kindUnknown: 'undefined',
    title: 'Statements',
    subtitle: 'Pick a statement to add it to your algorithm',
    search: 'Search statements…',
    empty: 'No matching statements',
    collapse: 'Collapse panel',
    expand: 'Expand panel',
    groups: {
      variables: 'Variables',
      io: 'Input and output',
      conditionals: 'Conditionals',
      loops: 'Loops',
      lists: 'Lists',
      functions: 'Functions',
      notes: 'Notes',
    },
  },
  listOps: {
    append: 'Add to the end',
    insert: 'Insert at',
    removeAt: 'Remove from',
    sort: 'Sort',
    reverse: 'Reverse',
    ascending: 'Smallest to largest',
    descending: 'Largest to smallest',
  },
  statements: {
    declare: { label: 'Create variable', hint: 'Store a value under a name' },
    assign: { label: 'Change variable', hint: 'Replace the stored value' },
    say: { label: 'Say', hint: 'The robot says something out loud' },
    ask: { label: 'Ask', hint: 'The robot requests a value and stores it' },
    if: { label: 'If… then', hint: 'Run statements only when something is true' },
    while: { label: 'While', hint: 'Repeat while the condition stays true' },
    repeat: { label: 'Repeat N times', hint: 'Repeat a fixed number of times' },
    forEach: { label: 'From… to', hint: 'Count from one number up to another' },
    listOp: { label: 'Change list', hint: 'Add, remove or sort items' },
    forEachItem: { label: 'For each item', hint: 'Walk a list from start to end' },
    function: { label: 'Function', hint: 'Group steps under a name and reuse them' },
    return: { label: 'Return', hint: 'Hand back a result and end the function' },
    call: { label: 'Call', hint: 'Run a function you already created' },
    comment: { label: 'Comment', hint: 'A note for the reader; it does not run' },
  },
  panels: {
    palette: 'Statements panel',
    drawer: 'Code and diagram panel',
    robot: 'Robot panel',
    canvas: 'Canvas view',
    resize: 'Drag to resize',
  },
  problems: {
    undefinedVariable: 'The variable "{name}" does not exist yet. Create it before this statement.',
    assignBeforeDeclare: 'You cannot change "{name}" because you have not created it yet.',
    noVariableChosen: 'A variable still needs to be chosen here.',
    emptyName: 'This variable needs a name.',
    invalidName: '"{name}" will not work as a name: start with a letter and use only letters, numbers and _.',
    reservedName: '"{name}" is a reserved word in the language. Pick another name.',
    duplicateName: 'A variable "{name}" already exists. Creating it again discards the previous value.',
    noFunctionChosen: 'Pick which function to call.',
    wrongArgumentCount: '"{name}" needs {expected} input(s) and you are passing {got}.',
    emptyParam: "One of this function's inputs has no name.",
    emptyFunction: 'This function does nothing yet. Add statements inside it.',
    returnOutsideFunction: '"Return" only means something inside a function.',
    kindMismatch:
      'The type of "{name}" does not match what it holds. The value still works, but the type says otherwise.',
    retypeLostContent:
      'Changing the type replaced some values that could not be converted. Undo (Ctrl+Z) to get them back.',
    emptySay: 'The robot will say nothing because the message is empty.',
    emptyAsk: 'The question is empty: the student will not know what to answer.',
    emptyBranches: 'This decision does nothing yet. Add statements inside it.',
    emptyLoop: 'This loop is empty: it will repeat nothing.',
    endlessLoop: 'The condition never changes inside the loop, so it will never end.',
    negativeTimes: 'You cannot repeat a negative number of times.',
    zeroStep: 'With a step of 0 the loop would never advance.',
    rangeNeverRuns: 'This range will not run even once: check from, to and the step.',
    title: 'Checks',
    none: 'All good',
    countOne: '1 thing to review',
    count: '{count} things to review',
  },
  tour: {
    replay: 'Show the tour again',
    skip: 'Skip',
    next: 'Next',
    back: 'Back',
    done: 'Start',
    welcomeTitle: 'Meet Tobot',
    welcomeBody: 'Here you build algorithms out of blocks and Tobot runs them step by step. A minute and you will know your way around.',
    paletteTitle: 'The statements',
    paletteBody: 'Drag or click a statement to add it. The variables you have created stay in view at the top, with their types.',
    canvasTitle: 'Your algorithm',
    canvasBody: 'Your algorithm, top to bottom. Drag to reorder, and drop inside a block to nest.',
    drawerTitle: 'The other views',
    drawerBody: 'The same algorithm written other ways, plus the console. Switch tabs to compare.',
    selectTitle: 'Select, then act',
    selectBody: 'Click a block to select it, or drag across the canvas for several. A bar appears with what to do with them, and the one you picked is described at the top right.',
    robotTitle: 'The robot',
    robotBody: 'It runs your algorithm and talks as it goes. If something breaks, it says what and where.',
    runTitle: 'Run it',
    runBody: 'Run it straight through or step by step. Stepping lets you go back whenever you like.',
    panelsTitle: 'Arrange your space',
    panelsBody: 'Hide the panels you do not need, or drag their edges. However you arrange it is how it comes back next time.',
    endTitle: 'Ready to start',
    endBody: 'That is it. Open an example or start from scratch — the guide stays here if you want it.',
  },
  home: {
    title: 'Your algorithms',
    subtitle:
      'Start a new one, pick up where you left off, or open a challenge or example and learn by solving.',
    newHint: 'Begin with a blank canvas',
    backToEditor: 'Back to the editor',
    sections: 'Sections',
    goHome: 'Home',
  },
  banners: {
    mineTitle: 'Your algorithms',
    mineBody: 'Start a new one, pick up where you left off, or bring in a file someone shared with you.',
    challengesTitle: 'Test yourself',
    challengesBody: 'Half-built exercises: the comments say what is missing and you write the rest.',
    examplesTitle: 'Learn from examples',
    examplesBody: 'Typical problems already solved. Open one and compare the different views to see how it works.',
    conceptsTitle: 'Understand the ideas',
    conceptsBody: 'Variables, decisions and loops explained from scratch, with references to go deeper.',
  },
  auth: {
    signInTitle: 'Sign in to your account',
    signInBody: 'Your algorithms follow you between the classroom and home.',
    signUpTitle: 'Create your account',
    signUpBody: 'With an account your work is saved and waiting on any computer.',
    email: 'Email',
    password: 'Password',
    emailHint: 'you@email.com',
    passwordHint: 'At least 6 characters',
    signIn: 'Sign in',
    signUp: 'Sign up',
    working: 'One moment…',
    or: 'or',
    withGoogle: 'Continue with Google',
    noAccount: 'No account yet?',
    haveAccount: 'Already have an account?',
    continueWithout: 'Continue without an account, on this browser only',
    checkEmail: 'We sent you an email to confirm your account. Open it and come back.',
    wrongCredentials: 'That email and password do not match.',
    alreadyExists: 'An account with that email already exists. Sign in instead.',
    passwordTooShort: 'The password needs at least 6 characters.',
    genericError: 'We could not complete that. Please try again.',
    signOut: 'Sign out',
    account: 'Your account',
    localOnly: 'Working on this browser only',
    signInPrompt: 'Sign in to save my work',
    firstName: 'First name',
    lastName: 'Last name',
    firstNameHint: 'Alex',
    lastNameHint: 'Taylor',
    signedInAs: 'Your account',
    signedOut: 'Signed out',
    localMode: 'Local mode',
    notConfigured: 'Your algorithms live in this browser',
    notConfiguredHint: 'Signing in needs Supabase connected. See the README.',
    forgotPassword: 'Forgot your password?',
    forgotTitle: 'Reset your password',
    forgotBody: 'Type your email and we will send you a link to choose a new one.',
    sendReset: 'Send the link',
    resetSent:
      'If there is an account with that email, the link to choose a new password is on its way to it.',
    backToSignIn: 'Back to signing in',
    newPasswordTitle: 'Choose a new password',
    newPasswordBody: 'Type it twice and you are straight back in with it.',
    newPassword: 'New password',
    confirmPassword: 'Repeat the password',
    confirmPasswordHint: 'The same one again',
    passwordsDiffer: 'The two passwords are not the same.',
    savePassword: 'Save the password',
    passwordUpdated: 'Done. Your password has been changed.',
    passwordSameAsOld: 'That is already your password. Choose a different one.',
    linkExpired: 'That link has expired or has already been used. Ask for a new one.',
    showPassword: 'Show the password',
    hidePassword: 'Hide the password',
  },

  classroom: {
    section: 'My classroom',
    lead: 'Gather your group, set exercises, and see how they are doing.',
    needsAccount: 'Classrooms need an account: it is what ties your work to your course.',

    createTitle: 'Create a classroom',
    createHint: 'For teachers. We give you a code to hand out.',
    createAction: 'Create the classroom',
    namePlaceholder: 'Course name',

    joinTitle: 'Join a classroom',
    joinHint: 'Type the code your teacher gave you.',
    joinAction: 'Join',
    codePlaceholder: 'Code',
    joinFailed: 'No classroom has that code.',

    code: 'Classroom code',
    copyCode: 'Copy the code',
    copied: 'Copied',
    teacherOf: 'You are the teacher',
    memberOf: 'You are enrolled',
    members: 'People',
    noMembers: 'Nobody has joined yet. Hand out the code.',
    remove: 'Remove from the classroom',
    leave: 'Leave the classroom',

    assignments: 'Assignments',
    noAssignments: 'No assignments yet.',
    newAssignment: 'New assignment',
    assignmentTitle: 'Title',
    points: 'Points',
    fromAlgorithm: 'Whatever you have open in the editor will be set.',
    cases: 'Test cases',
    casesHint: 'What to answer each question, and what the program should print. One line per answer.',
    caseAnswers: 'Answers',
    caseExpect: 'Should print',
    addCase: 'Add a case',
    removeCase: 'Remove',
    noCases: 'Without cases it cannot be marked automatically.',
    save: 'Save the assignment',
    cancel: 'Cancel',
    deleteAssignment: 'Delete the assignment',

    open: 'Open',
    handIn: 'Hand in',
    handedIn: 'Handed in',
    notHandedIn: 'Not handed in',
    yourScore: '{score} of {points}',
    pending: 'Not marked',

    grade: 'Mark everything',
    grading: 'Marking…',
    gradedCount: '{count} submissions marked',
    nothingToGrade: 'There is nothing to mark.',

    ranking: 'Leaderboard',
    rankPosition: 'Place',
    rankStudent: 'Student',
    rankPoints: 'Points',
    rankHandedIn: 'Handed in',
    noRanking: 'Once there are marked submissions, they show up here.',
    unnamed: 'Unnamed',
  },
  code: {
    tidy: 'Tidy the indentation',
    assignInCondition: 'A single "=" here stores instead of comparing, so the condition is always true. Use "===" to compare.',
    openAsCode: 'Open in JavaScript',
    errorAt: 'Line {line}: {message}',
    clearConsole: 'Clear the output',
    copy: 'Copy the code',
    copied: 'Copied',
    download: 'Download .js',
    goToProblem: 'Go to the warning',
    find: 'Find',
    wrap: 'Wrap lines',
    textBigger: 'Larger type',
    textSmaller: 'Smaller type',
    position: 'Line {line}, column {column}',
    problemsOne: '1 warning',
    problems: '{count} warnings',
    noProblems: 'No warnings',
    reference: 'What you can use',
    referenceHint: 'Hover one to learn more. Ctrl+Space writes it for you.',
    newCode: 'Write in JavaScript',
    newHint: 'A code document, for when you want to write it yourself',
    badge: 'JavaScript',
    starter: 'console.log("Hello!");\n',
    blank: 'Untitled code',
    run: 'Run',
    stop: 'Stop',
    running: 'Running…',
    onlyInTobot: 'Inside Tobot the robot asks. Outside it, "prompt" asks just the same.',
    tooLong: 'The program did not finish, so we stopped it. Check that some loop reaches its end.',
    missingAwait: 'This needs "await" in front. Without it you keep the question, not the answer.',
  },
  verbs: {
    forEachItem: 'for each',
    function: 'function',
    return: 'return',
    call: 'call',
    declare: 'variable',
    assign: 'change',
    say: 'say',
    ask: 'ask',
    if: 'if',
    while: 'while',
    repeat: 'repeat',
    forEach: 'from',
    comment: 'note',
  },
  editor: {
    title: 'My algorithm',
    empty: 'Your algorithm is empty',
    emptyHint: 'Add your first statement from the panel on the left',
    dropHere: 'Drop here',
    emptyBranch: 'No statements',
    then: 'then',
    elseIf: 'else if',
    otherwise: 'otherwise',
    do: 'do',
    statementCount: '{count} statements',
    statementCountOne: '1 statement',
  },
  operators: {
    groupArithmetic: 'Maths',
    groupComparison: 'Comparisons',
    groupLogical: 'Logic',
    '+': 'plus',
    '-': 'minus',
    '*': 'times',
    '/': 'divided by',
    '%': 'remainder of',
    '==': 'is equal to',
    '!=': 'is not equal to',
    '<': 'is less than',
    '<=': 'is at most',
    '>': 'is greater than',
    '>=': 'is at least',
    '&&': 'and also',
    '||': 'or else',
    glyphAnd: 'AND',
    glyphOr: 'OR',
  },
  fields: {
    aList: 'a list',
    anItem: 'an item of…',
    nameItFirst: 'name the function first',
    aResult: "a function's result",
    howMany: 'how many…',
    items: 'items',
    noFunctions: 'create a function first',
    needs: 'needs',
    addParam: 'another input',
    removeParam: 'Remove this input',
    noParams: 'no inputs',
    async: 'asynchronous',
    asyncHint: 'Works on its own while the program carries on',
    operation: 'Operation',
    theValue: 'the value',
    atPosition: 'at position',
    in: 'in',
    name: 'name',
    value: 'value',
    condition: 'condition',
    operator: 'operation',
    message: 'message',
    question: 'question',
    times: 'times',
    from: 'from',
    to: 'to',
    step: 'step',
    saveIn: 'save in',
    expect: 'data type',
    aValue: 'a value',
    aVariable: 'a variable',
  },
  kinds: {
    list: 'list',
    number: 'number',
    text: 'text',
    boolean: 'true or false',
  },
  booleans: {
    true: 'true',
    false: 'false',
  },
  tabs: {
    natural: 'Natural language',
    pseudocode: 'Pseudocode',
    code: 'Code',
    naturalShort: 'Natural',
    pseudocodeShort: 'Pseudo',
    blocks: 'Algorithm',
    flowchart: 'Flowchart',
    console: 'Console',
    concepts: 'Concepts',
  },
  panel: {
    languageSelect: 'Programming language',
    viewHint: 'The same idea, written several ways',
  },
  robot: {
    idle: 'Ready when you are',
    thinking: 'Thinking…',
    speaking: 'Speaking',
    asking: 'Waiting for your answer',
    done: 'Program finished',
    error: 'Something went wrong',
    greeting: 'Hi! Write an algorithm and I will run it.',
    answerPlaceholder: 'Type your answer…',
  },
  console: {
    title: 'Console',
    empty: 'No output yet. Run your algorithm to hear the robot.',
    seeAll: 'See all {count} lines',
    variablesTitle: 'Variables',
    variablesEmpty: 'No variables yet',
    outputLabel: 'Output',
    inputLabel: 'Answer',
  },
  flowchart: {
    title: 'Flowchart',
    empty: 'Add statements to see the flowchart',
    start: 'Start',
    end: 'End',
    yes: 'yes',
    no: 'no',
    legend: 'Legend',
    legendTerminal: 'Start / End',
    legendProcess: 'Process',
    legendDecision: 'Decision',
    legendIo: 'Input / Output',
    activeNode: 'Current step',
    fit: 'Fit',
    zoomIn: 'Zoom in',
    zoomOut: 'Zoom out',
  },
  runtime: {
    running: 'Running',
    paused: 'Paused',
    finished: 'Finished',
    stepOf: 'Step {current} of {total}',
    stepCount: 'Step {count}',
    waitingInput: 'Waiting for input',
    speed: 'Speed',
    speedSlow: 'Slow',
    speedNormal: 'Normal',
    speedFast: 'Fast',
  },
  errors: {
    undefinedVariable: 'The variable "{name}" does not exist yet. Create it before using it.',
    unknownFunction: 'There is no function called "{name}". Check the name, or create it.',
    tooDeep:
      'Calls nested {limit} deep and stopped. A function that calls itself needs a case that ends it.',
    tooManyTasks:
      'Too many tasks are running at once ({limit}). Check whether a loop is starting one each time round.',
    divisionByZero: 'Cannot divide by zero.',
    notAList: 'This is not a list. These instructions only work with a list.',
    listNotANumber: 'You cannot do arithmetic with a whole list. Did you mean one item, or how many it has?',
    indexOutOfRange:
      'Position {index} does not exist: the list has {length} items, so they run from 0 to {last}.',
    indexNotWhole: 'A position has to be a whole number, and {index} is not.',
    emptyList: 'The list is empty, so there is nothing to remove.',
    notANumber: '"{value}" is not a valid number.',
    infiniteLoop:
      'The loop ran {limit} times and was stopped. Check that the condition eventually becomes false.',
    emptyName: 'The variable name cannot be empty.',
    invalidName: 'The name must start with a letter and contain only letters, numbers and underscores.',
    duplicateName: 'A variable named "{name}" already exists.',
    reservedName: '"{name}" is a reserved word in the language.',
    emptyCondition: 'This condition is incomplete.',
    negativeTimes: 'The repeat count cannot be negative.',
    zeroStep: 'The step cannot be zero, the loop would never end.',
  },
  export: {
    title: 'Export',
    subtitle: 'Download your algorithm or share it',
    code: 'Source code',
    flowchartSvg: 'Flowchart (SVG)',
    flowchartPng: 'Flowchart (PNG)',
    algorithmJson: 'Algorithm (JSON)',
    copyToClipboard: 'Copy to clipboard',
    download: 'Download',
    includeComments: 'Include explanatory comments',
  },
  library: {
    blocks: 'Blocks',
    loading: 'Loading your algorithms…',
    title: 'My algorithms',
    empty: 'You have not saved any algorithms yet',
    saved: 'Saved',
    examples: 'Examples',
    examplesHint: 'Open them to see how typical problems are solved',
    lastEdited: 'Edited {date}',
    confirmDelete: 'Delete "{name}"? This cannot be undone.',
    import: 'Import',
    importHint: 'Open a .json file exported from Tobot',
    importError: 'That file could not be read. Is it an algorithm exported from Tobot?',
    challenges: 'Challenges',
    challengesHint: 'Exercises with parts to complete',
    /*
       Plain difficulty words, because that is what the badge has to say at a
       glance. Naming the stage instead ("Practicando") described what the
       student was doing and left the reader guessing what the scale even
       measured.

       Five of them rather than three: the levels stay anchored to what a
       challenge requires, so "Muy difícil" is reserved for work Tobot cannot
       set yet and nothing has to be re-ranked when it arrives.
    */
    level1: 'Very easy',
    level2: 'Easy',
    level3: 'Intermediate',
    level4: 'Hard',
    level5: 'Very hard',
    levelLabel: 'Difficulty',
    topicLabel: 'Topic',
    filterAll: 'All',
    filterAllTopics: 'All topics',
    filterLabel: 'Filter challenges',
    filterEmpty: 'No challenge matches that filter',
    filterClear: 'Clear filters',
    filterCount: '{count} of {total} challenges',
    filterCountOne: '1 of {total} challenges',
    levelHint: 'Difficulty {level} of 5',
    duplicateSuffix: 'copy',
  },
  concepts: {
    title: 'Concepts',
    subtitle: 'Learn the idea behind each statement',
    references: 'References',
    referencesHint: 'Trusted sources to go deeper',
    relatedStatements: 'Related statements',
    keyIdea: 'Key idea',
    commonMistakes: 'Common mistakes',
    example: 'Example',
    readingTime: '{minutes} min read',
  },
  settings: {
    title: 'Preferences',
    language: 'Language',
    theme: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    storage: 'Storage',
    storageLocal: 'This browser',
    storageHint: 'Your algorithms are saved in this browser.',
  },
  a11y: {
    mainNav: 'Main navigation',
    algorithmEditor: 'Algorithm editor',
    codePanel: 'Code panel',
    robotStage: 'Robot stage',
    dragHandle: 'Move statement',
    activeStep: 'Current algorithm step',
  },
};
