/**
 * Spanish is the source of truth for copy. `en.ts` is typed against this
 * object, so a missing or misspelled key fails the build instead of silently
 * falling back at runtime.
 */
export const es = {
  app: {
    name: 'Tobot',
    tagline: 'Aprende a programar conversando con un robot',
    untitled: 'Algoritmo sin título',
  },
  actions: {
    /* Spanish and English both need the singular spelled out; `{count}
       instrucciones` reads as a typo at one, which is the commonest case. */
    selectedOne: '1 instrucción',
    selectedCount: '{count} instrucciones',
    copyBlock: 'Copiar',
    cut: 'Cortar',
    paste: 'Pegar',
    insertBefore: 'Insertar antes',
    insertAfter: 'Insertar después',
    run: 'Ejecutar',
    stop: 'Detener',
    step: 'Paso a paso',
    next: 'Paso a paso',
    stepBack: 'Paso atrás',
    exitStepping: 'Salir del paso a paso',
    stepOne: 'Ejecuta una sola instrucción y espera',
    pause: 'Pausar',
    resume: 'Continuar',
    reset: 'Reiniciar',
    clearConsole: 'Limpiar la consola',
    clear: 'Limpiar',
    copy: 'Copiar',
    copied: 'Copiado',
    export: 'Exportar',
    delete: 'Eliminar',
    partActions: 'Con esta parte',
    group: 'Agrupar con la siguiente',
    /* Nombra la mitad del menú del lienzo que no actúa sobre un bloque, sino
       sobre el trabajo: por ahora agrupar, y luego lo que venga. */
    tools: 'Herramientas',
    /* La herramienta arma un modo, no hace nada de inmediato — sin esto el
       estudiante hace clic y parece que el menú no respondió. */
    groupTool: 'Agrupar partes',
    groupHint: 'Arrastra sobre las partes que van juntas',
    groupUnavailable: 'Necesitas una expresión de tres partes o más',
    groupCount: 'Agrupar {count} partes',
    ungroup: 'Quitar el grupo',
    undo: 'Deshacer',
    redo: 'Rehacer',
    duplicate: 'Duplicar',
    moveUp: 'Subir',
    moveDown: 'Bajar',
    addValue: 'Agregar valor',
    addOperand: 'otra parte',
    removeOperand: 'Quitar esta parte',
    addStatement: 'Agregar instrucción',
    addElseIf: 'agregar «si no, si»',
    addElse: 'Agregar «si no»',
    removeElse: 'Quitar «si no»',
    rename: 'Renombrar',
    newAlgorithm: 'Nuevo algoritmo',
    send: 'Enviar',
    close: 'Cerrar',
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    learnMore: 'Saber más',
    save: 'Guardar',
  },
  landing: {
    eyebrow: 'Programación desde cero',
    title: 'Programar, sin pelear con la sintaxis',
    lead: 'Arma algoritmos con bloques y míralos cobrar vida al instante.',
    tryIt: 'Empezar ahora',
    tryItLong: 'Entrar a Tobot',
    signIn: 'Entrar',
    noAccount: 'Sin instalar nada. Empieza en el navegador.',
    demoCaption: 'Tobot arma este algoritmo y lo ejecuta, igual que lo harás tú.',

    stepsTitle: 'Así funciona',
    stepsLead: 'Tres pasos, y el primero toma menos de un minuto.',
    steps: {
      build: {
        title: 'Arma tu algoritmo',
        body: 'Arrastra instrucciones desde el panel. Mientras armas con bloques no existe el error de sintaxis: solo encajan de formas que tienen sentido.',
      },
      compare: {
        title: 'Entiende qué hace',
        body: 'Lo ves en español mientras se escribe en código real. Aprendes a leerlo sin tener que escribirlo todavía.',
      },
      run: {
        title: 'Ejecútalo paso a paso',
        body: 'El robot va instrucción por instrucción. Puedes devolverte, ver las variables y entender dónde se torció.',
      },
    },

    featuresTitle: 'Todo para que entiendas, no para que memorices',
    features: {
        views: {
        title: 'De la idea al código',
        body: 'Escribes en español y lo ves convertirse en código real, paso a paso. La relación entre los dos se aprende sola.',
      },
      diagram: {
        title: 'Diagramas de flujo',
        body: 'Se dibujan solos con la notación de clase. Los exportas para tu informe o tu presentación.',
      },
      run: {
        title: 'Adelante y atrás',
        body: 'Ejecuta paso a paso, devuélvete cuando quieras y observa cómo cambia cada variable.',
      },
      robot: {
        title: 'Un robot que te responde',
        body: 'Habla, te pregunta y reacciona a lo que escribiste. Programar deja de ser un texto mudo.',
      },
      concepts: {
        title: 'Conceptos explicados',
        body: 'Cada tema trae una explicación corta y enlaces a fuentes confiables para profundizar.',
      },
      validation: {
        title: 'Errores en español',
        body: 'Tobot revisa mientras armas y te dice qué falta con palabras claras, no con jerga de compilador.',
      },
      write: {
        title: 'Y cuando estés listo, escríbelo',
        body: 'Abre tu algoritmo en JavaScript de verdad y sigue desde ahí. El robot te acompaña igual: ejecuta, pregunta y te dice en qué línea falló.',
      },
    },

    forTitle: '¿Para quién es?',
    forStudents: {
      title: 'Si estás aprendiendo',
      body: 'Entiendes qué hace tu código antes de memorizar cómo se escribe. Cuando pases a un lenguaje real, la lógica ya la tienes.',
    },
    forTeachers: {
      title: 'Si enseñas',
      body: 'Reparte ejercicios como un enlace, sin instalar nada en la sala. Tus estudiantes ven el diagrama y el código de lo mismo, lado a lado.',
    },


    robotHello: '¡Hola! Soy Tobot. Mira lo que puedo hacer.',
    robotDone: '¡Listo! Eso es un algoritmo.',
    robotGuide: 'Yo te acompaño en cada paso.',
    robotBye: '¿Empezamos?',

    sameTitle: 'Aprende programación de verdad',
    journey: {
      logic: 'Lógica',
      code: 'Código',
      concepts: 'Conceptos',
      beyond: 'Y lo que venga',
    },
    views: {
      natural: 'Español',
      pseudocode: 'Pseudocódigo',
      code: 'Código real',
      chart: 'Diagrama',
    },

    showcaseTitle: 'Escribes lógica, no sintaxis',
    showcaseBody: 'Piensas la solución y Tobot la escribe en código real, mientras aprendes a leerlo.',
    showcaseLabels: {
      blocks: 'Tus bloques',
      natural: 'En español',
      code: 'En código',
    },

    readingTime: '{minutes} min',
    boxLabel: 'Dentro de Tobot',
    andMore: 'Y más',
    sourcedFrom: 'Con fuentes de {publishers}.',
    exploreConcepts: 'Explorar los conceptos',
    learnTitle: 'Cada tema, explicado',
    learnLead: 'Mientras armas tu algoritmo, cada instrucción trae su explicación al lado: la idea que hay detrás, los errores comunes y a dónde ir para profundizar.',
    learn: {
      explained: 'Cada tema con su explicación, en palabras claras',
      sources: 'Enlaces a fuentes confiables para ir más a fondo',
      pace: 'A tu ritmo, sin nadie mirando por encima del hombro',
      growing: 'Temas nuevos a medida que avanzas',
    },
    closingTitle: '¿Armamos tu primer algoritmo?',
    closingBody: 'Abre el editor y empieza a jugar. Si más adelante quieres guardar tu trabajo y encontrarlo desde cualquier computador, creas tu cuenta.',
  },
  save: {
    /* At rest the work *is* saved — the last write finished. Same word
       as `saved` on purpose: the difference between them is the tick
       settling in, not the wording changing under the reader. */
    idle: 'Guardado',
    saving: 'Guardando…',
    saved: 'Guardado',
    error: 'Sin guardar',
  },
  /** The panel above the robot, describing whatever is selected. */
  inspector: {
    title: 'Selección',
    problemCount: '{count} cosas por revisar',
  },
  palette: {
    variables: 'Variables en uso',
    elementOf: 'elemento de {list}',
    kindUnknown: 'sin definir',
    title: 'Instrucciones',
    subtitle: 'Elige una instrucción para agregarla a tu algoritmo',
    search: 'Buscar instrucción…',
    empty: 'No hay instrucciones que coincidan',
    collapse: 'Contraer panel',
    expand: 'Expandir panel',
    groups: {
      variables: 'Variables',
      io: 'Entrada y salida',
      conditionals: 'Condicionales',
      loops: 'Ciclos',
      lists: 'Listas',
      functions: 'Funciones',
      notes: 'Notas',
    },
  },
  /** Names for the five ways a list can change, used by the picker on the
   * block. Verbs, because each one is something the student does to the list. */
  listOps: {
    append: 'Agregar al final',
    insert: 'Insertar en',
    removeAt: 'Quitar de',
    sort: 'Ordenar',
    reverse: 'Invertir',
    ascending: 'De menor a mayor',
    descending: 'De mayor a menor',
  },
  statements: {
    declare: { label: 'Crear variable', hint: 'Guarda un valor con un nombre' },
    assign: { label: 'Cambiar variable', hint: 'Reemplaza el valor guardado' },
    say: { label: 'Decir', hint: 'El robot dice algo en voz alta' },
    ask: { label: 'Preguntar', hint: 'El robot pide un dato y lo guarda' },
    if: { label: 'Si… entonces', hint: 'Ejecuta instrucciones solo si se cumple algo' },
    while: { label: 'Mientras', hint: 'Repite mientras la condición sea verdadera' },
    repeat: { label: 'Repetir N veces', hint: 'Repite una cantidad fija de veces' },
    forEach: { label: 'Desde… hasta', hint: 'Cuenta desde un número hasta otro' },
    listOp: { label: 'Cambiar lista', hint: 'Agrega, quita u ordena elementos' },
    forEachItem: { label: 'Para cada elemento', hint: 'Recorre una lista de principio a fin' },
    function: { label: 'Función', hint: 'Agrupa pasos bajo un nombre y reutilízalos' },
    return: { label: 'Devolver', hint: 'Entrega un resultado y termina la función' },
    call: { label: 'Llamar', hint: 'Ejecuta una función que ya creaste' },
    comment: { label: 'Comentario', hint: 'Una nota para quien lee; no se ejecuta' },
  },
  /** Short inline wording used inside blocks, where the palette's longer
   * labels ("Crear variable") would read awkwardly mid-sentence. */
  /** Names for the resizable/hideable regions, used by toggles and handles. */
  panels: {
    palette: 'Panel de instrucciones',
    drawer: 'Panel de código y diagrama',
    robot: 'Panel del robot',
    canvas: 'Vista del lienzo',
    resize: 'Arrastra para redimensionar',
  },
  /** Validation messages shown on the block itself. Each one names the fix,
   * not just the fault: a student should know what to do next. */
  problems: {
    undefinedVariable: 'La variable «{name}» no existe todavía. Créala antes de esta instrucción.',
    assignBeforeDeclare: 'No puedes cambiar «{name}» porque aún no la has creado.',
    noVariableChosen: 'Falta elegir una variable aquí.',
    emptyName: 'Esta variable necesita un nombre.',
    invalidName: '«{name}» no sirve como nombre: empieza con una letra y usa solo letras, números y _.',
    reservedName: '«{name}» es una palabra reservada del lenguaje. Elige otro nombre.',
    duplicateName: 'Ya existe una variable «{name}». Al crearla de nuevo pierdes el valor anterior.',
    noFunctionChosen: 'Falta elegir qué función llamar.',
    wrongArgumentCount: '«{name}» necesita {expected} dato(s) y le estás pasando {got}.',
    emptyParam: 'Uno de los datos de esta función no tiene nombre.',
    emptyFunction: 'Esta función no hace nada todavía. Agrega instrucciones dentro.',
    returnOutsideFunction: '«Devolver» solo tiene sentido dentro de una función.',
    kindMismatch:
      'El tipo de «{name}» no coincide con lo que guarda. El valor funciona igual, pero el tipo dice otra cosa.',
    retypeLostContent:
      'Al cambiar el tipo, algunos valores no se pudieron convertir y se reemplazaron. Deshaz (Ctrl+Z) si quieres recuperarlos.',
    emptySay: 'El robot no dirá nada porque el mensaje está vacío.',
    emptyAsk: 'La pregunta está vacía: el estudiante no sabrá qué responder.',
    emptyBranches: 'Esta decisión no hace nada todavía. Agrega instrucciones dentro.',
    emptyLoop: 'Este ciclo está vacío: repetirá nada.',
    endlessLoop: 'La condición no cambia dentro del ciclo, así que nunca terminará.',
    negativeTimes: 'No se puede repetir una cantidad negativa de veces.',
    zeroStep: 'Con paso 0 el ciclo nunca avanzaría.',
    rangeNeverRuns: 'Este recorrido no se ejecutará ni una vez: revisa desde, hasta y el paso.',
    title: 'Revisiones',
    none: 'Todo en orden',
    countOne: '1 detalle por revisar',
    count: '{count} detalles por revisar',
  },
  /** First-run tour. Each stop names one region and says what it is for. */
  tour: {
    replay: 'Ver la guía otra vez',
    skip: 'Saltar',
    next: 'Siguiente',
    back: 'Atrás',
    done: 'Empezar',
    welcomeTitle: 'Te presento a Tobot',
    welcomeBody: 'Aquí escribes algoritmos con bloques y Tobot los ejecuta paso a paso. Un minuto y sabes moverte.',
    paletteTitle: 'Las instrucciones',
    paletteBody: 'Arrastra o pulsa una instrucción para añadirla. Arriba están siempre a la vista las variables que ya creaste, con su tipo.',
    canvasTitle: 'Tu algoritmo',
    canvasBody: 'Tu algoritmo, de arriba abajo. Arrastra para reordenar y suelta dentro de un bloque para anidar.',
    drawerTitle: 'Las otras vistas',
    drawerBody: 'El mismo algoritmo escrito de otras formas, más la consola. Cambia de pestaña para comparar.',
    selectTitle: 'Selecciona y actúa',
    selectBody: 'Pulsa un bloque para seleccionarlo, o arrastra sobre el lienzo para varios. Aparecerá una barra con qué hacer con ellos, y arriba a la derecha el detalle del que elegiste.',
    robotTitle: 'El robot',
    robotBody: 'Ejecuta tu algoritmo y te habla mientras avanza. Si algo falla, te dice qué y dónde.',
    runTitle: 'Ejecutar',
    runBody: 'Ejecuta de corrido o paso a paso. En modo paso puedes devolverte cuando quieras.',
    panelsTitle: 'Acomoda tu espacio',
    panelsBody: 'Oculta los paneles que no necesites o arrastra sus bordes. Lo que dejes acomodado te espera así la próxima vez.',
    endTitle: 'Listo para empezar',
    endBody: 'Ya está. Abre un ejemplo o empieza en blanco: la guía sigue aquí si la necesitas.',
  },
  /** Landing view copy. */
  home: {
    title: 'Tus algoritmos',
    subtitle:
      'Empieza uno nuevo, retoma lo que dejaste, o abre un reto o un ejemplo para aprender resolviendo.',
    newHint: 'Empieza con el lienzo en blanco',
    backToEditor: 'Volver al editor',
    sections: 'Secciones',
    goHome: 'Inicio',
  },
  /** One banner per section: each says what that section is for. */
  banners: {
    mineTitle: 'Tus algoritmos',
    mineBody: 'Empieza uno nuevo, retoma lo que dejaste, o trae un archivo que te hayan compartido.',
    challengesTitle: 'Ponte a prueba',
    challengesBody: 'Ejercicios a medio hacer: los comentarios te dicen qué falta y tú escribes el resto.',
    examplesTitle: 'Aprende con ejemplos',
    examplesBody: 'Problemas típicos ya resueltos. Ábrelos y compara las distintas vistas para ver cómo funcionan.',
    conceptsTitle: 'Entiende las ideas',
    conceptsBody: 'Variables, decisiones y ciclos explicados desde cero, con referencias para profundizar.',
  },
  /** Account screen. Signing in is optional, and the copy says so. */
  auth: {
    signInTitle: 'Entra a tu cuenta',
    signInBody: 'Tus algoritmos te siguen entre el salón y tu casa.',
    signUpTitle: 'Crea tu cuenta',
    signUpBody: 'Con una cuenta, tu trabajo se guarda y lo encuentras desde cualquier computador.',
    email: 'Correo',
    password: 'Contraseña',
    emailHint: 'tu@correo.com',
    passwordHint: 'Mínimo 6 caracteres',
    signIn: 'Entrar',
    signUp: 'Registrarme',
    working: 'Un momento…',
    or: 'o',
    withGoogle: 'Continuar con Google',
    noAccount: '¿No tienes cuenta?',
    haveAccount: '¿Ya tienes cuenta?',
    continueWithout: 'Seguir sin cuenta, solo en este navegador',
    checkEmail: 'Te enviamos un correo para confirmar tu cuenta. Ábrelo y vuelve aquí.',
    wrongCredentials: 'El correo o la contraseña no coinciden.',
    alreadyExists: 'Ya existe una cuenta con ese correo. Entra en vez de registrarte.',
    passwordTooShort: 'La contraseña necesita al menos 6 caracteres.',
    genericError: 'No pudimos completar la acción. Inténtalo otra vez.',
    signOut: 'Cerrar sesión',
    account: 'Tu cuenta',
    localOnly: 'Trabajando solo en este navegador',
    signInPrompt: 'Entrar para guardar mi trabajo',
    firstName: 'Nombre',
    lastName: 'Apellido',
    firstNameHint: 'Juan David',
    lastNameHint: 'Pérez',
    signedInAs: 'Tu cuenta',
    signedOut: 'Sin sesión',
    localMode: 'Modo local',
    notConfigured: 'Tus algoritmos viven en este navegador',
    notConfiguredHint:
      'Para entrar con una cuenta hay que conectar Supabase. Mira el README.',
    forgotPassword: '¿Olvidaste tu contraseña?',
    forgotTitle: 'Recupera tu contraseña',
    forgotBody: 'Escribe tu correo y te enviamos un enlace para crear una nueva.',
    sendReset: 'Enviar el enlace',
    resetSent:
      'Si hay una cuenta con ese correo, ahí llegará el enlace para crear una contraseña nueva.',
    backToSignIn: 'Volver a entrar',
    newPasswordTitle: 'Crea una contraseña nueva',
    newPasswordBody: 'Escríbela dos veces y entras con ella de inmediato.',
    newPassword: 'Contraseña nueva',
    confirmPassword: 'Repite la contraseña',
    confirmPasswordHint: 'La misma de arriba',
    passwordsDiffer: 'Las dos contraseñas no son iguales.',
    savePassword: 'Guardar la contraseña',
    passwordUpdated: 'Listo. Tu contraseña quedó cambiada.',
    passwordSameAsOld: 'Esa ya es tu contraseña. Escribe una distinta.',
    linkExpired: 'Ese enlace ya venció o se usó. Pide uno nuevo.',
    showPassword: 'Mostrar la contraseña',
    hidePassword: 'Ocultar la contraseña',
  },

  classroom: {
    section: 'Mi salón',
    lead: 'Reúne a tu grupo, pon ejercicios y mira cómo van.',
    needsAccount: 'Los salones necesitan una cuenta: es lo que une tu trabajo con el de tu curso.',

    createTitle: 'Crear un salón',
    createHint: 'Para docentes. Te damos un código para repartir.',
    createAction: 'Crear el salón',
    namePlaceholder: 'Nombre del curso',

    joinTitle: 'Entrar a un salón',
    joinHint: 'Escribe el código que te dio tu profe.',
    joinAction: 'Entrar',
    codePlaceholder: 'Código',
    joinFailed: 'No hay ningún salón con ese código.',

    code: 'Código del salón',
    copyCode: 'Copiar el código',
    copied: 'Copiado',
    teacherOf: 'Eres el docente',
    memberOf: 'Estás inscrito',
    members: 'Personas',
    noMembers: 'Todavía no ha entrado nadie. Reparte el código.',
    remove: 'Sacar del salón',
    leave: 'Salir del salón',

    assignments: 'Asignaciones',
    noAssignments: 'Aún no hay asignaciones.',
    newAssignment: 'Nueva asignación',
    assignmentTitle: 'Título',
    points: 'Puntos',
    fromAlgorithm: 'Se pondrá lo que tengas abierto en el editor.',
    cases: 'Casos de prueba',
    casesHint: 'Qué se responde a cada pregunta, y qué debe mostrar el programa. Una línea por respuesta.',
    caseAnswers: 'Respuestas',
    caseExpect: 'Debe mostrar',
    addCase: 'Añadir caso',
    removeCase: 'Quitar',
    noCases: 'Sin casos no se puede calificar solo.',
    save: 'Guardar la asignación',
    cancel: 'Cancelar',
    deleteAssignment: 'Borrar la asignación',

    open: 'Abrir',
    handIn: 'Entregar',
    handedIn: 'Entregado',
    notHandedIn: 'Sin entregar',
    yourScore: '{score} de {points}',
    pending: 'Sin calificar',

    grade: 'Calificar todo',
    grading: 'Calificando…',
    gradedCount: '{count} entregas calificadas',
    nothingToGrade: 'No hay entregas que calificar.',

    ranking: 'Tabla de posiciones',
    rankPosition: 'Puesto',
    rankStudent: 'Estudiante',
    rankPoints: 'Puntos',
    rankHandedIn: 'Entregas',
    noRanking: 'Cuando haya entregas calificadas, aparecen aquí.',
    unnamed: 'Sin nombre',
  },
  code: {
    tidy: 'Ordenar la sangría',
    assignInCondition: 'Aquí un «=» guarda en vez de comparar, y la condición siempre se cumple. Para comparar usa «===».',
    openAsCode: 'Abrir en JavaScript',
    errorAt: 'Línea {line}: {message}',
    clearConsole: 'Limpiar la salida',
    copy: 'Copiar el código',
    copied: 'Copiado',
    download: 'Descargar .js',
    goToProblem: 'Ir al aviso',
    find: 'Buscar',
    wrap: 'Ajustar líneas',
    textBigger: 'Agrandar la letra',
    textSmaller: 'Achicar la letra',
    position: 'Línea {line}, columna {column}',
    problemsOne: '1 aviso',
    problems: '{count} avisos',
    noProblems: 'Sin avisos',
    reference: 'Lo que puedes usar',
    referenceHint: 'Pasa el ratón por una para saber más. Ctrl+Espacio la escribe por ti.',
    newCode: 'Escribir en JavaScript',
    newHint: 'Un documento de código, para cuando ya quieras escribirlo tú',
    badge: 'JavaScript',
    starter: 'console.log("¡Hola!");\n',
    blank: 'Código sin título',
    run: 'Ejecutar',
    stop: 'Detener',
    running: 'Ejecutando…',
    onlyInTobot: 'En Tobot pregunta el robot. Fuera de Tobot, «prompt» pregunta igual.',
    tooLong: 'El programa no terminó y lo detuvimos. Revisa que algún ciclo llegue a su fin.',
    missingAwait: 'Falta «await» delante. Sin él te quedas con la pregunta, no con la respuesta.',
  },
  verbs: {
    forEachItem: 'para cada',
    function: 'función',
    return: 'devolver',
    call: 'llamar',
    declare: 'variable',
    assign: 'cambiar',
    say: 'decir',
    ask: 'preguntar',
    if: 'si',
    while: 'mientras',
    repeat: 'repetir',
    forEach: 'desde',
    comment: 'nota',
  },
  editor: {
    title: 'Mi algoritmo',
    empty: 'Tu algoritmo está vacío',
    emptyHint: 'Agrega tu primera instrucción desde el panel de la izquierda',
    dropHere: 'Suelta aquí',
    emptyBranch: 'Sin instrucciones',
    then: 'entonces',
    elseIf: 'si no, si',
    otherwise: 'si no',
    do: 'hacer',
    statementCount: '{count} instrucciones',
    statementCountOne: '1 instrucción',
  },
  operators: {
    groupArithmetic: 'Cuentas',
    groupComparison: 'Comparaciones',
    groupLogical: 'Lógicos',
    '+': 'más',
    '-': 'menos',
    '*': 'por',
    '/': 'dividido',
    '%': 'resto de dividir',
    '==': 'es igual a',
    '!=': 'es distinto de',
    '<': 'es menor que',
    '<=': 'es menor o igual que',
    '>': 'es mayor que',
    '>=': 'es mayor o igual que',
    '&&': 'y además',
    '||': 'o bien',
    /* Los lógicos se muestran con su palabra, no con `∧` y `∨`: esos son los
       símbolos de la lógica formal, que un estudiante de primer semestre no ha
       visto todavía y lee como flechas sin punta. */
    glyphAnd: 'Y',
    glyphOr: 'O',
  },
  fields: {
    aList: 'una lista',
    anItem: 'un elemento de…',
    nameItFirst: 'ponle nombre a la función primero',
    aResult: 'el resultado de una función',
    howMany: 'cuántos tiene…',
    items: 'elementos',
    noFunctions: 'crea una función primero',
    needs: 'necesita',
    addParam: 'otro dato',
    removeParam: 'Quitar este dato',
    noParams: 'sin datos',
    async: 'asíncrona',
    asyncHint: 'Trabaja por su cuenta mientras el programa sigue',
    operation: 'Operación',
    theValue: 'el valor',
    atPosition: 'en la posición',
    in: 'en',
    name: 'nombre',
    value: 'valor',
    condition: 'condición',
    operator: 'operación',
    message: 'mensaje',
    question: 'pregunta',
    times: 'veces',
    from: 'desde',
    to: 'hasta',
    step: 'de a',
    saveIn: 'guardar en',
    expect: 'tipo de dato',
    aValue: 'un valor',
    aVariable: 'una variable',
  },
  kinds: {
    list: 'lista',
    number: 'número',
    text: 'texto',
    boolean: 'verdadero o falso',
  },
  booleans: {
    true: 'verdadero',
    false: 'falso',
  },
  tabs: {
    natural: 'Lenguaje natural',
    pseudocode: 'Pseudocódigo',
    code: 'Código',
    naturalShort: 'Natural',
    pseudocodeShort: 'Pseudo',
    blocks: 'Algoritmo',
    flowchart: 'Diagrama',
    console: 'Consola',
    concepts: 'Conceptos',
  },
  panel: {
    languageSelect: 'Lenguaje de programación',
    viewHint: 'La misma idea, escrita de varias formas',
  },
  robot: {
    idle: 'Listo cuando quieras',
    thinking: 'Pensando…',
    speaking: 'Hablando',
    asking: 'Esperando tu respuesta',
    done: 'Programa terminado',
    error: 'Algo salió mal',
    greeting: '¡Hola! Escribe un algoritmo y lo ejecutaré.',
    answerPlaceholder: 'Escribe tu respuesta…',
  },
  console: {
    title: 'Consola',
    empty: 'Aún no hay salida. Ejecuta tu algoritmo para ver qué dice el robot.',
    seeAll: 'Ver las {count} líneas',
    variablesTitle: 'Variables',
    variablesEmpty: 'Sin variables todavía',
    outputLabel: 'Salida',
    inputLabel: 'Respuesta',
  },
  flowchart: {
    title: 'Diagrama de flujo',
    empty: 'Agrega instrucciones para ver el diagrama',
    start: 'Inicio',
    end: 'Fin',
    yes: 'sí',
    no: 'no',
    legend: 'Convenciones',
    legendTerminal: 'Inicio / Fin',
    legendProcess: 'Proceso',
    legendDecision: 'Decisión',
    legendIo: 'Entrada / Salida',
    activeNode: 'Paso actual',
    fit: 'Ajustar',
    zoomIn: 'Acercar',
    zoomOut: 'Alejar',
  },
  runtime: {
    running: 'Ejecutando',
    paused: 'En pausa',
    finished: 'Finalizado',
    stepOf: 'Paso {current} de {total}',
    stepCount: 'Paso {count}',
    waitingInput: 'Esperando respuesta',
    speed: 'Velocidad',
    speedSlow: 'Lenta',
    speedNormal: 'Normal',
    speedFast: 'Rápida',
  },
  errors: {
    undefinedVariable: 'La variable «{name}» no existe todavía. Créala antes de usarla.',
    unknownFunction: 'No existe ninguna función llamada «{name}». Revisa el nombre o créala.',
    tooDeep:
      'Las llamadas se anidaron {limit} veces y se detuvo. Si una función se llama a sí misma, necesita un caso que la detenga.',
    tooManyTasks:
      'Hay demasiadas tareas en marcha a la vez ({limit}). Revisa si un ciclo está lanzando una en cada vuelta.',
    divisionByZero: 'No se puede dividir entre cero.',
    notAList: 'Esto no es una lista. Solo puedes usar estas instrucciones con una lista.',
    listNotANumber: 'No puedes hacer cuentas con una lista entera. ¿Querías un elemento, o cuántos tiene?',
    indexOutOfRange:
      'La posición {index} no existe: la lista tiene {length} elementos, así que van de 0 a {last}.',
    indexNotWhole: 'Una posición tiene que ser un número entero, y {index} no lo es.',
    emptyList: 'La lista está vacía, así que no hay nada que quitar.',
    notANumber: '«{value}» no es un número válido.',
    infiniteLoop:
      'El ciclo se repitió {limit} veces y se detuvo. Revisa que la condición llegue a ser falsa.',
    emptyName: 'El nombre de la variable no puede estar vacío.',
    invalidName:
      'El nombre debe empezar con una letra y solo puede tener letras, números y guiones bajos.',
    duplicateName: 'Ya existe una variable llamada «{name}».',
    reservedName: '«{name}» es una palabra reservada del lenguaje.',
    emptyCondition: 'Esta condición está incompleta.',
    negativeTimes: 'La cantidad de repeticiones no puede ser negativa.',
    zeroStep: 'El paso no puede ser cero, el ciclo nunca terminaría.',
  },
  export: {
    title: 'Exportar',
    subtitle: 'Descarga tu algoritmo o compártelo',
    code: 'Código fuente',
    flowchartSvg: 'Diagrama (SVG)',
    flowchartPng: 'Diagrama (PNG)',
    algorithmJson: 'Algoritmo (JSON)',
    copyToClipboard: 'Copiar al portapapeles',
    download: 'Descargar',
    includeComments: 'Incluir comentarios explicativos',
  },
  library: {
    blocks: 'Bloques',
    loading: 'Cargando tus algoritmos…',
    title: 'Mis algoritmos',
    empty: 'Todavía no has guardado ningún algoritmo',
    saved: 'Guardados',
    examples: 'Ejemplos',
    examplesHint: 'Ábrelos para ver cómo se resuelven problemas típicos',
    lastEdited: 'Editado {date}',
    confirmDelete: '¿Eliminar «{name}»? Esta acción no se puede deshacer.',
    import: 'Importar',
    importHint: 'Abre un archivo .json exportado desde Tobot',
    importError: 'No se pudo leer ese archivo. ¿Es un algoritmo exportado desde Tobot?',
    challenges: 'Retos',
    challengesHint: 'Ejercicios con partes por completar',
    /*
       Plain difficulty words, because that is what the badge has to say at a
       glance. Naming the stage instead ("Practicando") described what the
       student was doing and left the reader guessing what the scale even
       measured.

       Five of them rather than three: the levels stay anchored to what a
       challenge requires, so "Muy difícil" is reserved for work Tobot cannot
       set yet and nothing has to be re-ranked when it arrives.
    */
    level1: 'Muy fácil',
    level2: 'Fácil',
    level3: 'Intermedio',
    level4: 'Difícil',
    level5: 'Muy difícil',
    levelLabel: 'Dificultad',
    topicLabel: 'Tema',
    filterAll: 'Todas',
    filterAllTopics: 'Todos los temas',
    filterLabel: 'Filtrar retos',
    filterEmpty: 'Ningún reto coincide con ese filtro',
    filterClear: 'Quitar filtros',
    filterCount: '{count} de {total} retos',
    filterCountOne: '1 de {total} retos',
    levelHint: 'Dificultad {level} de 5',
    duplicateSuffix: 'copia',
  },
  concepts: {
    title: 'Conceptos',
    subtitle: 'Aprende la idea detrás de cada instrucción',
    references: 'Referencias',
    referencesHint: 'Fuentes confiables para profundizar',
    relatedStatements: 'Instrucciones relacionadas',
    keyIdea: 'Idea clave',
    commonMistakes: 'Errores comunes',
    example: 'Ejemplo',
    readingTime: '{minutes} min de lectura',
  },
  settings: {
    title: 'Preferencias',
    language: 'Idioma',
    theme: 'Tema',
    themeLight: 'Claro',
    themeDark: 'Oscuro',
    themeSystem: 'Sistema',
    storage: 'Almacenamiento',
    storageLocal: 'Este navegador',
    storageHint: 'Tus algoritmos se guardan en este navegador.',
  },
  a11y: {
    mainNav: 'Navegación principal',
    algorithmEditor: 'Editor de algoritmo',
    codePanel: 'Panel de código',
    robotStage: 'Escenario del robot',
    dragHandle: 'Mover instrucción',
    activeStep: 'Paso actual del algoritmo',
  },
} as const;

/** Widens the literal types from `as const` so other locales can supply their
 * own strings while the key structure stays enforced. */
type Widen<T> = T extends string ? string : { [K in keyof T]: Widen<T[K]> };

export type Dictionary = Widen<typeof es>;
